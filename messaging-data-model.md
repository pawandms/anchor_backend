...
erDiagram
    ...
    USER_PRIVACY_SETTINGS {
        ...
        Boolean readReceipts "Enable/disable read receipts"
        String groups "EVERYONE, CONTACTS, NOBODY"
        String calls "EVERYONE, CONTACTS, NOBODY"
    }
    
    USER_NOTIFICATION_SETTINGS {
        Boolean messageNotifications "Enable message notifications"
        Boolean groupNotifications "Enable group notifications"
        Boolean callNotifications "Enable call notifications"
        Boolean statusNotifications "Enable status notifications"
        Boolean notificationSound "Play notification sound"
        Boolean vibration "Enable vibration"
        Boolean inAppNotifications "Show in-app notifications"
        Boolean showPreview "Show message preview"
        String notificationTone "Custom notification tone"
    }
```

## Embedded Object Structures

### ChannelSettings (Embedded in CHANNELS)
```javascript
{
  onlyAdminsCanPost: Boolean,      // Restrict posting to admins only
  onlyAdminsCanEdit: Boolean,      // Restrict channel info editing to admins
  membersCanAddOthers: Boolean,    // Allow members to add participants
  postPermission: Boolean,         // General post permission
  approveNewMembers: Boolean,      // Require admin approval for new members
  whoCanSeeMembers: String,        // "EVERYONE" or "ADMINS" (ChannelPermission enum)
  allowComments: Boolean,          // Enable comments on posts
  allowLikes: Boolean              // Enable likes/reactions
}
```

### User Privacy Settings (Embedded in USER)
```javascript
{
  ...
  readReceipts: Boolean,          // Enable/disable read receipts
  groups: String,                 // "EVERYONE", "CONTACTS", "NOBODY"
  calls: String                   // "EVERYONE", "CONTACTS", "NOBODY"
}
```

### User Notification Settings (Embedded in USER)
```javascript
{
  messageNotifications: Boolean,   // Enable message notifications
  groupNotifications: Boolean,     // Enable group notifications
  callNotifications: Boolean,      // Enable call notifications
  statusNotifications: Boolean,    // Enable status update notifications
  notificationSound: Boolean,      // Play notification sound
  vibration: Boolean,              // Enable vibration for notifications
  inAppNotifications: Boolean,     // Show in-app notifications
  showPreview: Boolean,            // Show message preview in notifications
  notificationTone: String         // Custom notification tone identifier
}
```

**Note:** In the Java model, `privacySettings` and `notificationSettings` are stored as `Map<String, Object>` to allow flexible key-value pairs. The structures above represent the recommended schema for these maps.
