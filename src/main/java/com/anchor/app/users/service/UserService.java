...
public class UserService {
    ...
    public void validateStructuralErrors(SignUpRequest request, BindingResult bindingResult) {
        try {
            ...
            if (bindingResult.hasErrors()) {
                ...
                bindingResult.getFieldErrors().forEach(error -> request.getErrors().add(new ErrorMsg(
                        error.getCode(),
                        error.getField(),
            ...
            request.setValid(false);
            request.getErrors().add(new ErrorMsg(ValidationErrorType.Invalid_Request.name(), "general",
        ...
        try {
            ...
            if (userRepository.existsByEmail(request.getEmail())) {
                ...
                request.getErrors().add(
                        new ErrorMsg(ValidationErrorType.Email_Already_Present.name(), "email", request.getEmail()));
            }
            ...
            if (request.getUserName() != null && !request.getUserName().isEmpty()) {
                if (userRepository.existsByUserName(request.getUserName())) {
                    ...
                    request.setValid(false);
                    request.getErrors().add(new ErrorMsg(ValidationErrorType.UserName_Already_Taken.name(), "userName",
                            request.getUserName()));
                }
            }
            ...
            if (request.getMobile() != null && !request.getMobile().isEmpty()) {
                if (userRepository.existsByMobile(request.getMobile())) {
                    ...
                    request.setValid(false);
                    request.getErrors().add(
                            new ErrorMsg(ValidationErrorType.Mobile_Already_Present.name(), "mobile",
                                    request.getMobile()));
                }
            }
        } catch (Exception e) {
     ...
     * Register a new user
     * 
     * @param request       SignUpRequest containing user details - response fields
     *                      will be populated
     ...
     * 
     * @param request UserProfile request object containing userId and other
     *                validation data
    ...
    public void getUserProfileById(UserProfile request) throws UserServiceException {
        try {
            ...
            // Check permissions - reqUserID will be populated from authenticated user in
            // AuthService
    ...
    public void addUpdateUserProfileImage(StreamMediaInfo req) throws UserServiceException {
        ...
        try {
            ...
            // Perform Authorization - reqUserID will be populated from authenticated user
            // in AuthService
     ...
     * 
     * @param userId          User ID
     * @param privacySettings UserPrivacy object containing privacy settings to
     *                        update
     * @throws UserServiceException if update fails
     */
    public void updateUserPrivacySettings(String userId, com.anchor.app.users.dto.UserPrivacy privacySettings)
     ...
     * 
     * @param userId               User ID
     * @param notificationSettings UserNotification object containing notification
     *                             settings to update
     * @throws UserServiceException if update fails
     */
    public void updateUserNotificationSettings(String userId,
            com.anchor.app.users.dto.UserNotification notificationSettings) throws UserServiceException {
        try {
    ...
    private void performUserInfoUpdate(String userId, UserInfoUpdateRequest request) throws UserServiceException {
        try {
            // Use the user object already fetched during validation to avoid additional DB
            // call
            ...
            // Prepare values for update - use new values if provided, otherwise keep
            // current values
            ...
            // Handle email update - when email is updated, userName is also updated with
            // the same value
            ...
            // Handle userLanguage update
            UserLanguageType userLanguage = currentUser.getUserLanguage();
