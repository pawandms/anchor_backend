...
public class UserController {
    ...
    public ResponseEntity<?> getUserProfile(
        ...
        try {
            ...
            if (!req.getErrors().isEmpty()) {
                ...
                response = new ResponseEntity<>("get User Profile Error Msg:" + e.getMessage(),
     ...
     * @return ProfileImageResponse with upload details
     */
    @PostMapping(value = "{userId}/media/profile-image", consumes = MediaType.MULTIPART_FORM_DATA_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> uploadProfileImage(
        ...
        try {
            ...
            if (!req.getErrors().isEmpty()) {
                ...
                response = new ResponseEntity<>("set User Profile Error Msg:" + e.getMessage(),
                        HttpStatus.INTERNAL_SERVER_ERROR);
            }
     ...
     */
    @PostMapping(value = "{userId}/privacy-settings", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     */
    @PostMapping(value = "{userId}/notification-settings", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     */
    @PostMapping(value = "{userId}/user-info", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    /**
     * Get recommended friends for the user
     * 
     * @param userId User ID from path
     * @return RecommendationProfile containing list of suggestions
     */
    ...
    public ResponseEntity<?> getRecommendations(
        ...
        try {
            ...
            if (req.isValid()) {
                ...
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(req);
            }
        } catch (Exception e) {
    ...
    /**
     * Refresh recommended friends for the user
     * 
     * @param userId User ID from path
     * @return Success message
     */
    @PostMapping(value = "{userId}/recommendations/refresh", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> refreshRecommendations(
        ...
        try {
            logger.info("Refreshing recommendations for userId: {}", userId);
            recommendationService.refreshRecommendations(req);

            if (req.isValid()) {
                Map<String, Object> successResponse = new HashMap<>();
                successResponse.put("success", true);
                successResponse.put("message", "Recommendations refreshed successfully");
                return ResponseEntity.status(HttpStatus.OK).body(successResponse);
            } else {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(req);
            }
        } catch (Exception e) {
            logger.error("Error refreshing recommendations for userId: {}", userId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
                    "success", false,
